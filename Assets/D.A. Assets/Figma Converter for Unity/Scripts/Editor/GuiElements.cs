...
namespace DA_Assets.FCU
{
    internal class GuiElements : Editor
    {
        ...
        public void DrawGroup(Group group)
        {
            switch (group.GroupType)
            {
                ...
                case GroupType.Hamburger:
                    if (HamburgerButton(group.Label, ref FigmaConverterUnity.Instance.itemsBuffer[(int)group.Fold]))
                    {
                        group.Body.Invoke();
                    }
                    GUILayout.EndVertical();
        ...
        public T EnumField<T>(GUIContent label, T @enum, bool uppercase = true)
        {
        ...
        public void SerializedPropertyField(string field, SerializedObject so)
        {
            DrawGroup(new Group
            {
                GroupType = GroupType.Horizontal,
